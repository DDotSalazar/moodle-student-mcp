  ...
  it('calls core_calendar_get_calendar_upcoming_view', async () => {
    ...
    mswServer.use(
      http.post(WS_ENDPOINT, async ({ request }) => {
        ...
        return HttpResponse.json({ events: [{ id: 1, name: 'Lecture', description: '', eventtype: 'course', timestart: 1751000000, timeduration: 3600, courseid: 7 }] });
      }),
    );
...
describe('getActionEvents', () => {
  it('calls core_calendar_get_action_events_by_timesort with the right window and limit', async () => {
    ...
    mswServer.use(
      http.post(WS_ENDPOINT, async ({ request }) => {
        capturedBody = await request.text();
        return HttpResponse.json({ events: [{ id: 99, name: 'HW1 due', description: '', modulename: 'assign', instance: 100, timestart: 1751000000, course: { id: 7, fullname: 'CS101', shortname: 'CS101' }, action: { name: 'Submit assignment', url: 'https://...', itemcount: 1, actionable: true } }] });
      }),
    );
    const client = new MoodleClient(MOODLE_URL, MOODLE_TOKEN);
